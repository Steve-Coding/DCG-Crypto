...
*Setup instructions for a gitian build of Daily Crypto Giveaways using a VM or physical system.*

Gitian is the deterministic build process that is used to build the DailyCrypto
...
to the DailyCrypto GitHub release page.
...
- [Building Daily Crypto Giveaways](#building-dailycrypto-core)
...
Building Daily Crypto Giveaways
--------------------
...
```bash
gpg --output ${VERSION}-linux/${NAME}/dailycrypto-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/dailycrypto-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/dailycrypto-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/dailycrypto-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/dailycrypto-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/dailycrypto-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/dailycrypto-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@dailycrypto.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/dailycrypto-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/dailycrypto-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/dailycrypto-osx-*-build.assert
```
