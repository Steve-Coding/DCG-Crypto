...
- [Development guidelines](#development-guidelines)
    - [General Daily Crypto Giveaways](#general-dailycrypto-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Daily Crypto Giveaways uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play DCGs (tDCG)" on the test network, if you
...
Daily Crypto Giveaways is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/dailycrypto-project/dailycrypto/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_dailycrypto
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_dailycrypto --log_level=test_suite
$ valgrind -v --leak-check=full src/dailycryptod -printtoconsole
```
...
# A coverage report will now be accessible at `./test_dailycrypto.coverage/index.html`.
```
...
- BitcoinMiner : Generates DCGs (if wallet is enabled).
...
However, in open source software such as Daily Crypto Giveaways, where everyone uses
...
pay attention to for reviewers of Daily Crypto Giveaways code.

General Daily Crypto Giveaways
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Daily Crypto Giveaways uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Daily Crypto Giveaways so that they can be integrated
...
file descriptors it will cause problems with DailyCrypto's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:DailyCrypto-Project/DailyCrypto.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `dailycrypto-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `dailycrypto-cli` and the GUI debug console use this table to determine how to
