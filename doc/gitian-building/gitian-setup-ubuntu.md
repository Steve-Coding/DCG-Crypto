...
The build script we'll be using is contained in the DailyCrypto github repository ([contrib/gitian-build.py](https://github.com/dailycrypto-project/dailycrypto/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the DailyCrypto repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/DailyCrypto-Project/DailyCrypto/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, dailycrypto-detached-sigs, and dailycrypto GitHub repos.
