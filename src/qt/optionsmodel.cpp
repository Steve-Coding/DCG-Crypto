...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::DCG);
    ...
    if (settings.contains("nAnonymizeDailyCryptoAmount") || reset)
        SoftSetArg("-anonymizedailycryptoamount", settings.value("nAnonymizeDailyCryptoAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in dailycrypto.cpp during shotdown to also remove the window positions
