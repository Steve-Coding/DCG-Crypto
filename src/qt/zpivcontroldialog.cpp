    ...
    model(0)
{
    ...
    // Title
    ui->labelTitle->setText(tr("Select zDCG Denominations to Spend"));
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to spend zDCG.";
            else if (!mint.isSeedCorrect)
                strReason = "The zDCG seed used to mint this zDCG is not the same as currently hold in the wallet";
            else
