...
#include "qt/dailycrypto/send.h"
#include "qt/dailycrypto/forms/ui_send.h"
#include "qt/dailycrypto/addnewcontactdialog.h"
#include "qt/dailycrypto/qtutils.h"
#include "qt/dailycrypto/sendchangeaddressdialog.h"
#include "qt/dailycrypto/optionbutton.h"
#include "qt/dailycrypto/sendconfirmdialog.h"
...
SendWidget::SendWidget(DailyCryptoGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (DCG)"));
    setCssProperty(ui->labelSubtitle1, "text-subtitle");

    /* Address */
    ui->labelSubtitleAddress->setText(tr("DailyCrypto address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 DCG");
...
void SendWidget::onSendClicked(){
    ...
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendPiv ? AskPassphraseDialog::Context::Send_DCG : AskPassphraseDialog::Context::Send_zDCG, true)){
...
bool SendWidget::send(QList<SendCoinsRecipient> recipients){
    ...
    if (currentTransaction.getTotalTransactionAmount() == 5000.0001234 * COIN)
        warningStr = tr("WARNING: This amount is exactly in value as masternode collateral\n and will be locked for one year. You may proceed if value is \n for masternode collateral. Please modify value if it is not for \nmasternode collateral to avoid coins being locked.\n");
...
bool SendWidget::sendZpiv(QList<SendCoinsRecipient> recipients){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zDCG is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zDCG selector if applicable
    ...
    if (walletModel->sendZpiv(
        ...
        inform(tr("zDCG transaction sent!"));
        ZPivControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZDCG_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zDCG require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zDCG transaction
        ...
        emit message("zDCG transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isDCG){
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            inform(tr("You don't have any zDCG in your balance to select."));
        }
    }
}
...
void SendWidget::onDCGSelected(bool _isDCG){
    isDCG = _isDCG;
    setCssProperty(coinIcon, _isDCG ? "coin-icon-piv" : "coin-icon-piv");
    refreshView();
