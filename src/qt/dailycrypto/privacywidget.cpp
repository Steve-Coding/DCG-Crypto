...
#include "qt/dailycrypto/privacywidget.h"
#include "qt/dailycrypto/forms/ui_privacywidget.h"
#include "qt/dailycrypto/qtutils.h"
#include "guiutil.h"
#include "qt/dailycrypto/denomgenerationdialog.h"
...
PrivacyWidget::PrivacyWidget(DailyCryptoGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zDCG anonymizes your DCG by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zDCG or convert back to DCG"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 DCG ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zDCG");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zDCG");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zDCG");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zDCG");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zDCG");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zDCG");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zDCG");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zDCG");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zDCG Movements"));
    ...
    // Only Convert to DCG enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzDCG->setTitleClassAndText("btn-title-grey", "Total 0 zDCG");
    ui->btnTotalzDCG->setSubTitleClassAndText("text-subtitle", "Show denominations of zDCG owned.");
    ui->btnTotalzDCG->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select DCG outputs to mint into zDCG.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zDCG");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzDCG, SIGNAL(clicked()), this, SLOT(onTotalZpivClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zDCG");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of DCG to mint into zDCG"));
    }else{
        btnText = tr("Convert back to DCG");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zDCG to convert back into DCG"));
    }
...
void PrivacyWidget::onTotalZpivClicked(){
    ...
    if(!isVisible){
        ...
        ui->btnTotalzDCG->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzDCG->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zDCG is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zDCG, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zDCG").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zDCG minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZpiv(
        ...
        // Spend succeed
        inform(tr("zDCG converted back to DCG"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::onCoinControlClicked(){
    if(ui->pushRight->isChecked()) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any DCG to select."));
        }
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zDCG </b>";
    ...
    ui->btnTotalzDCG->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
