...
#include "qt/dailycrypto/forms/ui_dashboardwidget.h"
#include "qt/dailycrypto/sendconfirmdialog.h"
#include "qt/dailycrypto/txrow.h"
...
DashboardWidget::DashboardWidget(DailyCryptoGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of DCG staked."));
    ...
    ui->labelAmountPiv->setText("0 DCG");
    ...
    ui->btnHowTo->setText(tr("How to get DCG"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("DCG")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isPiv = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZDCG;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair DCG, zDCG
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("DCG");
    set1 = new QBarSet("zDCG");
    set0->setColor(QColor(24,66,123));
