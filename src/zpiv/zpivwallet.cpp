// Copyright (c) 2017-2019 The DailyCrypto developers
...
CzDCGWallet::CzDCGWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZDCGSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZDCGSeed_deprecated()) {
                    LogPrintf("%s: Updated zDCG seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzDCGWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZDCGCount(nCountLastUsed);
    else if (!walletdb.ReadZDCGCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzDCGWallet::Lock()
{
    seedMaster = 0;
}

void CzDCGWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzDCGWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZDCG(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzDCGWallet::LoadMintPoolFromDB()
{
...
void CzDCGWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzDCGWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzDCGWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzDCGWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZDCG(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzDCGWallet::SeedToZDCG(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzDCGWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzDCGWallet::GenerateDeterministicZDCG(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzDCGWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzDCGWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzDCGWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
