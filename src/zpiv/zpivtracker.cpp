// Copyright (c) 2018-2020 The DailyCrypto developers
...
CzDCGTracker::CzDCGTracker(std::string strWalletFile)
{
...
CzDCGTracker::~CzDCGTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzDCGTracker::Init()
{
...
bool CzDCGTracker::Archive(CMintMeta& meta)
{
...
bool CzDCGTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzDCGTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzDCGTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzDCGTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzDCGTracker::GetSerialHashes()
{
...
CAmount CzDCGTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzDCGTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzDCGTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzDCGTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzDCGTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzDCGTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzDCGTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzDCGTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzDCGTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzDCGTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzDCGTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzDCGWallet* zDCGWallet)
{
    bool iszDCGWalletInitialized = (NULL != zDCGWallet);
    CMintMeta meta;
    ...
    if (! iszDCGWalletInitialized)
        zDCGWallet = new CzDCGWallet(strWalletFile);
    meta.isSeedCorrect = zDCGWallet->CheckSeed(dMint);
    if (! iszDCGWalletInitialized)
        delete zDCGWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzDCGTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzDCGTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzDCGTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzDCGTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzDCGTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzDCGTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzDCGWallet* zDCGWallet = new CzDCGWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zDCGWallet);
        }
        delete zDCGWallet;
