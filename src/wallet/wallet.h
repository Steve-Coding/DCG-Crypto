...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 DCG at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zDCG send
enum ZerocoinSpendStatus {
    ZDCG_SPEND_OKAY = 0,                            // No error
    ZDCG_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZDCG_WALLET_LOCKED = 2,                         // Wallet was locked
    ZDCG_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZDCG_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZDCG_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZDCG_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZDCG_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZDCG_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZDCG_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZDCG_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZDCG_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZDCG_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZDCG_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZDCG_SPENT_USED_ZDCG = 14,                      // Coin has already been spend
    ZDCG_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZDCG_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZDCGOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzDCGTracker> zpivTracker;
    ...
    void setZWallet(CzDCGWallet* zwallet);
    CzDCGWallet* getZWallet();
    ...
    /// Get 10000 DCG output and keys which can be used for the Masternode
    ...
    /** zDCG reset */
    boost::signals2::signal<void()> NotifyzDCGReset;
